...
function getUserId() {
  if (
    hermes_context.hasOwnProperty("kakao") &&
...
function getDeliveryFeeEditResult(
  dong,
  delivery_min,
  delivery_fee_edit,
  delivery_free
  ...
  try {
    ...
    let this_policy = "";
    if (
      task === "배달 지역 동 단위 추가/삭제" &&
      location_type === "배달 지역 삭제"
    ) {
      this_policy = dong;
    } else {
      this_policy =
    ...
    if (!isEmpty(result)) {
      if (result.hasOwnProperty("entityStatus") === true) {
        ...
        log(
          "getDeliveryFeeEditResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
      }
  ...
  try {
    ...
    if (!isEmpty(result)) {
      if (result.hasOwnProperty("entityStatus") === true) {
        ...
        log(
          "getExitYogiyoResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
      }
...
function getDeviceReturnResult(
  device_phone,
  device_contact_time,
  device_reason,
  device_address
  ...
  try {
    ...
    if (!isEmpty(result)) {
      if (result.hasOwnProperty("entityStatus") === true) {
        ...
        log(
          "getDeviceReturnResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
      }
...
function getDeviceOpenResult(device_phone, device_contact_time) {
  ...
  try {
    ...
    for (let i = 0; i < companies.length; ++i) {
      ...
      if (isEmpty(result)) {
        log(
          "getDeviceOpenResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
  ...
  try {
    ...
    if (!isEmpty(result)) {
      if (result.hasOwnProperty("entityStatus") === true) {
        ...
        log(
          "getCancelOrderResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
      }
...
function getYogiyoImageResult(image_type, image_detail) {
  ...
  try {
    ...
    var result = YogiyoImage(
      "17",
      telno,
      ceo_name,
      shop_name,
      bl_number,
      image_map[image_type],
      details
    );
    debugInfos.push({
      timestamp: now,
    ...
    if (!isEmpty(result)) {
      if (result.hasOwnProperty("entityStatus") === true) {
        ...
        log(
          "getYogiyoImageResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
      }
...
function getDeleteMenuResult() {
  ...
  try {
    ...
    for (let i = 0; i < companies.length; ++i) {
      ...
      if (isEmpty(result)) {
        log(
          "getSetOffTimeResult function result : webhook result is not valid"
        );
        setGlobal("debug_infos", debugInfos);
        return "false";
      } else if (!result.hasOwnProperty("entityStatus")) {
...
function getPageButtons(type, items, page) {
  ...
  if (previous) {
    message +=
      ...
      " 보기> 버튼을 눌러주세요.";
    buttons.push(
      createTextButton("이전 " + type + " 보기", "이전 " + type + " 보기")
    );
  }
  for (let i = begin; i < end; ++i) {
    var items_name = items[i].invisible
      ? "[품절] " + items[i].name
      : items[i].name;
...
function changePage(type, delta) {
  ...
  try {
    ...
    if (
      (deltaNum > 0 && position.next) ||
      (deltaNum < 0 && position.previous)
    ) {
      if (type === "category") {
  ...
  answers.push(
    createCardFormat(
      ...
      [
        ...
        createTextButton(
          "내 배달 지역 확인",
          "https://owner.yogiyo.co.kr/owner/delivery/"
        ),
      ]
    )
  );
...
function selectCompanyNumber() {
  ...
  if (com_num_start == "미수집" || com_num_start == "0") {
    ...
    message =
      "조회된 사업자등록번호 개수가 9건을 초과하여, 나머지 번호를 보여드릴게요.";
  }

  message +=
    "\n\n" +
    task +
    " 접수에 사용할 사업자등록번호를 아래 버튼 중에서 선택해주세요.";
  //message += '\n\n✅ 모든 사업자등록번호에 동일하게 적용하시려면 <모두 선택>을 선택해주세요.'

  for (var i = Number(com_num_start); i < Number(com_num_end); i++) {
    list_buttons.push(
      createTextButton(list_company_number[i], list_company_number[i])
    );
  }
...
function selectRestaurantName() {
  ...
  if (res_name_start == "미수집" || res_name_start == "0") {
    ...
    message =
      "조회된 상호명 개수가 9건을 초과하여, 나머지 번호를 보여드릴게요.";
  }
  ...
  for (var i = Number(res_name_start); i < Number(res_name_end); i++) {
    list_buttons.push(
      createTextButton(company_names[i], i + 1 + "번 사업체 선택")
    );
  }
...
function checkRestaurantNameCount() {
  ...
  try {
    ...
    for (var j = 0; j < list_company.length; j++) {
      if (
        list_company[j].hasOwnProperty("company_number") &&
        list_company[j]["company_number"] == company_number
      ) {
...
function sayMenuAndPrice(resay) {
  const link = {
    mobile:
      "https://owner.yogiyo.co.kr/owner/app-download?deeplink=ygyownerapp://restaurant/out_of_dish",
...
function sayBusinessHourAndClosedDay(resay) {
  const link = {
    mobile:
      "https://owner.yogiyo.co.kr/owner/app-download?deeplink=ygyownerapp://oe_pause",
...
function sayAboutOrder(resay) {
  const link = {
    mobile:
      "https://owner.yogiyo.co.kr/owner/app-download?deeplink=ygyownerapp://orderlist",
  ...
  answers.push(
    createCardFormat(
      ...
      [
        ...
        createTextButton(
          "주문전달 핸드폰 번호 추가",
          "https://owner.yogiyo.co.kr/owner/profile/"
        ),
      ]
    )
  );
...
function sayCategoryList() {
  ...
  if (task.indexOf("추가") != -1) {
    message +=
      "추가할 메뉴가 속할 카테고리를 아래 버튼 중에서 선택해주세요.\n단, 주류 메뉴 카테고리는 챗봇을 통해서 접수하실 수 없어요.";
  ...
  const { message: buttonMessage, buttons } = getPageButtons(
    "카테고리",
    categories,
    categoryPage
  );
...
function saveDeliveryPolicy(
  ...
  if (
    task === "배달 지역 동 단위 추가/삭제" &&
    getGlobal("location_type") === "배달 지역 삭제"
  ) {
    policy = dong;
  } else {
    policy =
      dong + "," + delivery_min + "," + delivery_fee_edit + "," + delivery_free;
  }
  ...
  if (user_says.length >= Number(num)) {
    if (keyword != "" && keyword == user_says) {
      ...
      if (
        ...
        annotations[0].hasOwnProperty("begin_pos") === true
      ) {
        for (var i = 0; i < annotations.length; i++) {
          var name = annotations[i]["entity_name"];

          if (name == "사업자등록번호" || name == "확인 요청 선택지") {
            return "false";
          }
        }
        return "true";
...
function convertPrice() {
  ...
  function convertPrice(price) {
    if (price == "없음" || price == "무료" || price == "설정 안함") {
      ...
      var message =
        String(Number(price).toLocaleString("ko-KR")) +
        "원(" +
        convertKorean(Number(price)) +
        "원)";
      return message;
    }
  }
...
function checkCompanyNumber(company_number) {
  try {
    ...
    for (var i = 0; i < list_company.length; i++) {
      ...
      this_company_number = this_company_number
        .replace("-", "")
        .replace("-", "");
...
function sayOrderNumber() {
  ...
  for (var i = 0; i < last; i++) {
    ...
    if (!isEmpty(order_number)) {
      ...
      body_text +=
        "\n주문 금액 : " +
        convertPrice(order_list[i]["total_price"].toString()) +
        "원";
      body_text += "\n주문 내역 : " + order_list[i]["ordered_items"];

      list_card.push(
        createCard(title, body_text, "", [
          createTextButton("이 주문번호 취소", order_number + "번 주문 취소"),
        ])
      );
    }
  }
...
function checkSerialNumber() {
  ...
  try {
    var result = checkSerialResultCode(
      "T60I03",
      "ccsj4hjzTQpASYun/40Z+umZKHJz/CRo0kt7fvkr/0RmsWmvgzDje5hruoGUoZMgfmz6FKBGx9HChD+rfrlPiA==",
      "010-7318-2113",
      "20210601",
      "20210901"
    );
    for (var i = 0; i < result.length; i++) {
      ...
      order_list.push(
        createCardFormat("", "return : " + serial_no + "\n" + itm_cd, "", [])
      );
    }
